...
class DeepSeekV2LiteDecodeEmbedding(BaseModule):
    """The embedding module of DeepSeekV2-Lite."""
...
class DeepSeekV2LiteDecodeAttn(BaseModule):
    """The attention module of DeepSeekV2-Lite using Multi-Latent Attention (MLA)."""
    def __init__(self, config: Config):
        super().__init__(config)
        self.attn_bs = config.attn_bs
        self._build_ops()
...
class DeepSeekV2LiteDecodeMLP(BaseModule):
    """The MLP module of DeepSeekV2-Lite."""
    def __init__(self, config: Config):
    ...
    def _build_ops(self):
        ...
        self.mlp_swiglu = OpSwiglu(bs, 2 * self.model_config.moe_intermediate_size, self.aichip_config)
...
class DeepSeekV2LiteDecodeMoe(BaseModule):
    """The MoE module of DeepSeekV2-Lite."""
    def __init__(self, config: Config):
        super().__init__(config, hw_type="ffn")
...
class DeepSeekV2LiteDecodeLMHead(BaseModule):
    """The LMHead module of DeepSeekV2-Lite."""
    def __init__(self, config: Config):
        super().__init__(config)
        self.attn_bs = config.attn_bs
        self._build_ops()
