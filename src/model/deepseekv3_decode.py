...
class DeepSeekV3DecodeAttn(BaseModule):
    ...
    def _aggregate_times(self):
        ...
        if self.config.serving_mode == "AFD":
            logging.error(
                f"p2p a2e time:{self.a2e_send.e2e_time * 1e6:.1f}us,"
                f"p2p a2e time:{self.a2e_recv.e2e_time * 1e6:.1f}us,"
            )
...
class DeepSeekV3DecodeMoe(BaseModule):
    ...
    def _aggregate_times(self):
        ...
        if self.config.serving_mode == "AFD":
            ...
            logging.error(
                f"p2p e2a time:{self.e2a_recv.e2e_time * 1e6:.1f}us,"
            )
        logging.error(f"MoE - dispatch: {self.dispatch_time*1e6:.1f}us, compute: {self.e2e_time*1e6:.1f}us, combine: {self.combine_time*1e6:.1f}us")
